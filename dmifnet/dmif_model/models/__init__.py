...
from dmifnet.dmif_model.models import encoder_latent, decoder
...
class OccupancyNetwork(nn.Module):
    ...
    def forward(self, p, inputs, sample=True, **kwargs):
        ...
        '''
        batch_size = p.size(0)
        c0, c1, c2, cdog = self.encode_inputs(inputs)
        z = self.get_z_from_prior((batch_size,), sample=sample)
        p_r0, p_r1, p_r2, p_r3, p_r4 = self.decode(p, z, c0, c1, c2, cdog, **kwargs)
        return p_r0, p_r1, p_r2, p_r3, p_r4
    ...
    def compute_elbo(self, p, occ, inputs, **kwargs):
        ...
        '''
        c0, c1, c2, cdog = self.encode_inputs(inputs)
        q_z = self.infer_z(p, occ, c0, **kwargs)
        z = q_z.rsample()
        p_r0, p_r1, p_r2, p_r3, p_r4 = self.decode(p, z, c0, c1, c2, cdog, **kwargs)

        rec_error = -p_r4.log_prob(occ).sum(dim=-1)
    ...
    def decode(self, p, z, c0, c1, c2, cdog, **kwargs):
        ...
        logits0, logits1, logits2, logits3, logits4 = self.decoder(p, z, c0, c1, c2, cdog, **kwargs)
        p_r0 = dist.Bernoulli(logits=logits0)
        p_r1 = dist.Bernoulli(logits=logits1)
        p_r2 = dist.Bernoulli(logits=logits2)
        p_r3 = dist.Bernoulli(logits=logits3)
        p_r4 = dist.Bernoulli(logits=logits4)
        return p_r0, p_r1, p_r2, p_r3, p_r4
